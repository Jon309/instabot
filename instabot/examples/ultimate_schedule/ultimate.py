...
# Like hashtags from hashtag_database.txt
schedule.every(8).hours.do(run_threaded, like_hashtags)

# Like timeline
schedule.every(2).hours.do(run_threaded, like_timeline)

# At 4pm like followers from username_database.txt
schedule.every(1).days.at("17:00").do(run_threaded, like_followers_from_random_user_file)

# At 11am follow followers from username_database.txt
schedule.every(2).days.at("20:00").do(run_threaded, follow_followers)

# Comments on timeline media with comments from comments.txt
# schedule.every(16).hours.do(run_threaded, comment_medias)

# At 8am unfollow non-followers
schedule.every(1).days.at("08:00").do(run_threaded, unfollow_non_followers)

# Follow users who post hashtag from hashtag_database.txt
schedule.every(12).hours.do(run_threaded, follow_users_from_hashtag_file)

# Comment on posts with hashtags from hashtag_database.txt with comments from comments.txt
# schedule.every(6).hours.do(run_threaded, comment_hashtag)

# Uploads pictures from /photos/ with random captions from photo_captions.txt
# schedule.every(1).days.at("21:28").do(run_threaded, upload_pictures)

# Puts non-followers on blacklist
